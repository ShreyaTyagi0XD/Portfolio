...
function scrollHeader(){
    ...
    //if scroll > 80 viewport height , add the scroll-header class to the header tag
    if(this.scrollY >= 80)
...
//(if user selected)Previously selected topic
const selectedTheme = localStorage.getItem('selected-theme')
const selectedIcon = localStorage.getItem('selected-icon')

//we obtain the current theme that the interface has by validating the dark-theme
const getCurrentTheme = ()=> document.body.classList.contains(darkTheme) ? 'dark' : 'light'
const getCurrentIcon = ()=> themeButton.classList.contains(darkTheme) ? 'uil-moon' : 'uil-sun'
